...
export function SubjectTypeView({
  subject,
  onBack,
  onSelectType,
}: {
  subject: any;
  onBack: () => void;
  onSelectType: (t: "lessons" | "exercises") => void;
}) {
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-center gap-3 md:gap-4 mb-2 md:mb-4">
        <button
          onClick={onBack}
          className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl glass hover:bg-white flex items-center justify-center text-slate-600 transition-all font-bold hover:scale-[1.05] active:scale-95"
        >
          <ChevronRight size={18} className="md:w-5 md:h-5" />
        </button>
        <div
          className={`w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl ${subject.bg} ${subject.color} flex items-center justify-center shadow-sm`}
        >
          <subject.icon size={16} className="md:w-5 md:h-5" />
        </div>
        <div>
          <h2 className="font-bold text-base md:text-xl text-slate-800">
            {subject.name}
          </h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 gap-3 md:gap-6">
        ...
        >
          ...
          <h3 className="font-bold text-lg md:text-2xl text-slate-800 mb-1 md:mb-2 group-hover:text-blue-700 transition-colors">
            الدروس
          </h3>
          <p className="text-[10px] md:text-sm text-slate-500 leading-tight">
            مشاهدة الدروس والملخصات
          </p>
        </div>
        ...
        >
          ...
          <h3 className="font-bold text-lg md:text-2xl text-slate-800 mb-1 md:mb-2 group-hover:text-emerald-700 transition-colors">
            التمارين
          </h3>
          <p className="text-[10px] md:text-sm text-slate-500 leading-tight">
            حل تمارين تطبيقية مع التصحيح
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-center gap-3 md:gap-4 mb-2 md:mb-4">
        <button
          onClick={onBack}
          className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl glass hover:bg-white flex items-center justify-center text-slate-600 transition-all font-bold"
        >
          <ChevronRight size={18} className="md:w-5 md:h-5" />
        </button>
        <div
          className={`w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-indigo-100 text-indigo-600 flex items-center justify-center shadow-sm`}
        >
          <subject.icon size={16} className="md:w-5 md:h-5" />
        </div>
        <div>
      ...
      <div className="space-y-8">
        {[1, 2, 3].map((trimestreNum) => {
          const trimUnits = subject.units.filter(
            (u: any) =>
              u.trimestre === trimestreNum ||
              (!u.trimestre && trimestreNum === 1),
          );
          ...
          return (
            <div key={`trim-${trimestreNum}`} className="space-y-4">
              <h3 className="font-bold text-lg md:text-xl text-slate-800 flex items-center gap-2">
                <span
                  className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm ${trimestreNum === 1 ? "bg-indigo-100 text-indigo-600" : trimestreNum === 2 ? "bg-purple-100 text-purple-600" : "bg-orange-100 text-orange-600"}`}
                >
                  {trimestreNum}
                </span>
                الفصل{" "}
                        ...
                        <div className="flex justify-between items-start">
                          <div className="w-full pl-6">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-[10px] md:text-xs font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md">
                                {progress}%
                              </span>
                            </div>
                            <h3 className="font-bold text-sm md:text-lg text-slate-800 group-hover:text-blue-600 transition-colors line-clamp-1">
                              {unit.name}
                            </h3>
                            <div className="flex gap-3 md:gap-4 mt-2 md:mt-4 text-xs md:text-sm text-slate-500 font-medium">
                              ...
                              <span className="flex items-center gap-1 md:gap-1.5">
                                ...
                                />{" "}
                                {unit.exercises?.length || 0} تمارين
                              </span>
                            </div>
...
export function UnitDetailsView({
  ...
  unit: any;
  onBack: () => void;
  onSelectItem: (type: "lessons" | "exercises", item: any) => void;
}) {
  const [activeTab, setActiveTab] = useState<"lessons" | "exercises">(
    "exercises",
  );
  const items =
    activeTab === "lessons" ? unit.lessons || [] : unit.exercises || [];
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="glass rounded-3xl md:rounded-[2rem] p-3 md:p-6 shadow-sm">
        {items.length === 0 ? (
          <div className="text-center py-10 text-slate-500 font-bold text-sm md:text-base">
          ...
          <div className="space-y-2 md:space-y-3">
            {activeTab === "lessons" ? (
              items.map((item: any, idx: number) => (
                <div
                  key={item.id}
                  className="flex flex-col sm:flex-row sm:items-center justify-between p-3 md:p-5 bg-white/70 hover:bg-white rounded-2xl md:rounded-3xl border border-slate-100 transition-all group"
                >
                  <div className="flex items-center gap-3 md:gap-4 mb-3 sm:mb-0">
                    <div
                      className={
                        "w-8 h-8 md:w-10 md:h-10 shrink-0 rounded-full flex items-center justify-center font-bold text-xs md:text-sm shadow-sm bg-blue-50 text-blue-600 border border-blue-100"
                      }
                    >
                      {idx + 1}
                    </div>
                    <div>
                      ...
                      <p className="text-[10px] md:text-xs font-bold text-slate-400 mt-1 md:mt-1.5 flex items-center gap-1 md:gap-1.5">
                        <ClipboardList
                          size={10}
                          className="text-emerald-500 md:w-3 md:h-3"
                        />
                  ...
                  >
                    ...
                    <ChevronLeft
                      size={14}
                      className="opacity-70 md:w-4 md:h-4"
                    />
                  </button>
                </div>
              ))
                ...
                {items.map((item: any, idx: number) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="text-[10px] md:text-xs font-bold text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded-md">
                        تمرين {idx + 1}
                      </span>
                    </div>
                    <h3 className="font-bold text-sm md:text-lg text-slate-800 group-hover:text-emerald-600 transition-colors line-clamp-2 leading-tight flex-1">
                      {item.title}
                    </h3>
    ...
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-center justify-between mb-2 md:mb-4">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            <h2 className="font-bold text-base md:text-xl text-slate-800">
              {isLessons ? "قائمة الدروس" : "قائمة التمارين"}
            </h2>
            <p className="text-[10px] md:text-xs text-slate-500 font-medium">
              {unit.name}
            </p>
        ...
        <div
          className={`w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl ${isLessons ? "bg-blue-100 text-blue-600" : "bg-emerald-100 text-emerald-600"} flex items-center justify-center shadow-sm`}
        >
          {isLessons ? (
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="glass rounded-3xl md:rounded-[2rem] p-4 md:p-8">
        <div
          className="prose prose-sm md:prose-base prose-slate max-w-none text-right overflow-hidden break-words w-full"
          dir="rtl"
        >
            ...
            <ReactMarkdown
              ...
              components={{
                table: ({ node, children, ...props }: any) => (
                  <div
                    className="overflow-x-auto w-full mb-6 relative"
                    dir="auto"
                  >
                    <table
                      {...props}
                      className="w-full text-center border-collapse border border-slate-300"
                    >
                ...
                colgroup: ({ node, children, ...props }: any) => (
                  <colgroup {...props}>
                    {React.Children.toArray(children).filter(
                      (c: any) => typeof c !== "string" || c.trim() !== "",
                    )}
                ...
                th: ({ node, ...props }: any) => (
                  <th
                    {...props}
                    className="border border-slate-300 px-4 py-2 bg-slate-50 font-bold"
                  />
                ),
                td: ({ node, ...props }: any) => (
                  <td
...
export function InteractiveExerciseView({
  ...
  const [isCompleted, setIsCompleted] = useState(() => {
    return (
      exercise?.id && getProgressSync("completed_exercise", exercise.id) > 0
    );
  });
  ...
  const getInitialCount = () => {
    ...
    return parseInt(
      localStorage.getItem(`exercise_gen_count_${exercise.id}`) || "0",
      10,
    );
  };
  ...
  const generateNewExercise = async () => {
    const isVip = getProgressSync("system", "is_vip") > 0;
    if (!isVip && genCount >= 2) {
      if (onPay) onPay();
      return;
    }
    ...
    if (supabase) {
      ...
      if (data && data.api_key) {
        apiKey = data.api_key;
        aiModel =
          data.ai_model && data.ai_model !== "gemini-2.5-flash"
            ? data.ai_model
    ...
    if (!apiKey) {
      setErrorMsg(
        "الرجاء إعداد مفتاح Gemini API من صفحة الإعدادات (في لوحة الإدارة) أولاً.",
      );
      return;
    }
    ...
    try {
      ...
      if (response.text) {
        ...
        if (exercise?.id) {
          localStorage.setItem(
            `exercise_gen_count_${exercise.id}`,
            newCount.toString(),
          );
        }
      } else {
      ...
      if (errMsg.includes("504") || errMsg.includes("503"))
        errMsg = "الخادم يواجه ضغطاً (503/504). المحاولة لاحقاً.";
      else if (errMsg.includes("Failed to fetch"))
        errMsg = "انقطع الاتصال بالإنترنت أو الخادم أثناء التحليل.";
      else if (errMsg.includes("token limit"))
        errMsg = "تجاوز التوليد الحد الأقصى للنصوص المسموح بها.";
      else if (
        errMsg.includes("429") ||
        ...
        errMsg =
          "لقد استنفدت الحصة المجانية لمفتاح Gemini API هذا (Quota Exceeded). يرجى التحقق من خطة الدفع الخاصة بك أو إضافة مفتاح API جديد.";
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 print:hidden gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          >
            <CheckCircle size={16} />{" "}
            <span>{showAnswers ? "إخفاء الحل" : "عرض الحل"}</span>
          </button>
        </div>
      </div>
      ...
      <div
        className="bg-white mx-auto shadow-sm border border-slate-200 print:shadow-none print:border-none p-6 md:p-12 lg:p-16 text-slate-900 border-t-[12px] border-t-slate-800 relative z-10 font-[Traditional_Arabic,serif] text-base md:text-lg"
      ...
      >
          ...
          >
            {preprocessMath(
              currentExercise.exam
                ?.replace(/\\n/g, "\n")
                .replace(
                  /([^\n])\s+([أبتثجحخدذرزسشصضطظعغفقكلمنهوي]\))/g,
                  "$1\n\n$2",
                ),
            )}
          </ReactMarkdown>
        ...
        {showAnswers && currentExercise.solution && (
          <div
            ref={solutionRef}
            className="mt-8 border-t-2 border-emerald-500 pt-8 animate-in fade-in slide-in-from-bottom-4 duration-500"
          >
            <div className="text-center mb-6">
              <h3 className="text-xl font-bold text-emerald-700 bg-emerald-50 inline-block px-6 py-2 rounded-full border border-emerald-200">
                التصحيح النموذجي
              </h3>
            </div>
            <div className="markdown-body rtl prose max-w-none text-right overflow-hidden break-words w-full">
              ...
              {!isCompleted && !showSuccessAnim && (
                <button
                  onClick={() => {
                    ...
                    if (exercise?.id) {
                      if (
                        getProgressSync("completed_exercise", exercise.id) === 0
                      ) {
                        addXP(15);
                      }
