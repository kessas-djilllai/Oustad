...
export default function AdminUsers() {
  ...
  const [userXpMap, setUserXpMap] = useState<
    Record<string, { id: number; xp: number }>
  >({});
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (usersError) {
        if (
          usersError.code === "42883" ||
          usersError.message.includes("Could not find the function")
        ) {
      ...
      if (error) {
        if (
          error.message.includes(
            "function update_user_meta_by_id does not exist",
          )
        ) {
          throw new Error(
            "يجب تحديث قاعدة البيانات وتشغيل كود الـ SQL الخاص بتعديل الاسم.",
          );
        }
        throw error;
      }
  ...
  const handleDelete = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        if (
          error.message.includes("function delete_user_by_id does not exist")
        ) {
    ...
    return (
      <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 mb-8">
        ...
        <div className="bg-orange-50 text-orange-800 p-4 rounded-2xl text-sm leading-relaxed mb-6">
          لعرض وإدارة المستخدمين، يجب تشغيل أكواد SQL التالية في لوحة تحكم
          Supabase (SQL Editor):
        </div>
        <div
          className="bg-slate-900 text-slate-100 p-4 rounded-xl text-left font-mono text-xs overflow-x-auto whitespace-pre-wrap leading-relaxed"
          dir="ltr"
        >
...
-- 3. إنشاء دالة أو وظيفة لجلب المتصدرين بأسماءهم الحقيقية بشكل آمن (ضروري للوحة الشرف)
create or replace function public.get_public_leaderboard()
returns table(user_id uuid, progress_value int, full_name text) as $$
begin
  return query select p.user_id, p.progress_value, (u.raw_user_meta_data->>'full_name')::text
  from public.user_progress p
  join auth.users u on u.id = p.user_id
  where p.item_type = 'gamification' and p.item_id = 'xp'
  order by p.progress_value desc limit 100;
end;
$$ language plpgsql security definer;
grant execute on function public.get_public_leaderboard() to anon, authenticated;

-- 4. إنشاء دالة أو وظيفة لحذف المستخدم وصلاحيات الـ API لها
...
`}
        ...
        <button
          onClick={fetchUsers}
          className="mt-6 w-full lg:w-auto px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-all"
        >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            العدد الإجمالي: {users.length} مستخدم
          </p>
        </div>
        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors w-full md:w-auto shrink-0 shadow-sm shadow-blue-500/20"
          >
            <Plus size={18} />
            إضافة مستخدم
          </button>
          ...
          <table className="w-full text-right border-separate border-spacing-y-2">
            <thead>
              <tr className="text-sm text-slate-500">
                <th className="font-bold py-2 px-4 whitespace-nowrap">
                  الاسم والتخصص
                </th>
                <th className="font-bold py-2 px-4 whitespace-nowrap">
                  البريد الإلكتروني
                ...
                <th className="font-bold py-2 px-4 text-left whitespace-nowrap">
                  إجراءات
                </th>
              </tr>
            </thead>
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((user) => {
                  ...
                  return (
                    <tr
                      key={user.id}
                      className="bg-slate-50 hover:bg-slate-100 transition-colors rounded-2xl group"
                    >
                        ...
                        {editingName === user.id ? (
                          <div className="flex flex-col gap-2">
                            ...
                            <div className="flex gap-2">
                              <button
                                onClick={() => handleSaveMeta(user.id)}
                                className="w-6 h-6 bg-emerald-500 hover:bg-emerald-600 text-white flex items-center justify-center rounded"
                              >
                                <Check size={14} />
                              </button>
                              <button
                                onClick={() => setEditingName(null)}
                                className="w-6 h-6 bg-slate-300 hover:bg-slate-400 text-slate-700 flex items-center justify-center rounded text-xs font-bold"
                              >
                                ✕
                              </button>
                            </div>
                          ...
                          <div
                            className="flex justify-between items-start group/name cursor-pointer"
                            onClick={() => {
                              ...
                              setTempSpec(
                                user.raw_user_meta_data?.specialization ||
                                  "علوم تجريبية",
                              );
                            }}
                          >
                            ...
                            <Edit2
                              size={12}
                              className="opacity-0 group-hover/name:opacity-100 mt-1 text-slate-400 hover:text-blue-500 transition-opacity"
                            />
                          </div>
                        )}
                      </td>
                      ...
                      <td
                        className="py-3 px-4 text-sm font-bold text-slate-600 border-y border-slate-100 group-hover:border-slate-200"
                        dir="ltr"
                      >
                        {editingXp === user.id ? (
                          ...
                          <div
                            className="flex justify-end gap-2 items-center text-amber-500 group/xp cursor-pointer"
                            onClick={() => {
                              setEditingXp(user.id);
                              setTempXp(userXp.toString());
                            }}
                          >
                            <span>{userXp}</span>{" "}
                            ...
                            <Edit2
                              size={12}
                              className="opacity-0 group-hover/xp:opacity-100 text-slate-400 hover:text-blue-500 mix-blend-multiply"
                            />
                          </div>
                        )}
                      </td>
              ...
              <form onSubmit={handleAddUser} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full border-slate-200 rounded-xl p-3 bg-slate-50 border outline-none focus:border-blue-500 focus:bg-white"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full border-slate-200 rounded-xl p-3 bg-slate-50 border outline-none focus:border-blue-500 focus:bg-white text-right"
                    dir="rtl"
                  >
                    <option value="علوم تجريبية">علوم تجريبية</option>
                    <option value="تقني رياضي">تقني رياضي</option>
            ...
            >
              ...
              <div className="flex flex-col items-center text-center mb-6 mt-2">
                ...
                <p className="text-slate-600 text-sm leading-relaxed">
                  أنت على وشك حذف المستخدم <br />
                  <span className="font-bold text-slate-800">
                    {deletingUser.raw_user_meta_data?.full_name ||
                      deletingUser.email}
                  </span>
                  .<br /> لا يمكن التراجع عن هذا الإجراء وسيتم مسح جميع بياناته.
                </p>
              </div>
              ...
              <form onSubmit={handleDelete} className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">
                    للتأكيد، يرجى كتابة{" "}
                    <span className="text-red-500 font-mono select-none">
                      delete
                    </span>{" "}
                    في الحقل أدناه:
                  </label>
